<html>
    ...
    <body>
       ...
       <%
        ...
        while(rst.next())
        {
            mfcc=rst.getDouble("MFCC");
        }
        %> <h2 style="color:#339999;text-align:justify">
                   <% out.print("Songs in mood: "+p); %>
               </h2>
                <%  rsf=smt3.executeQuery("SELECT * FROM FEATURE WHERE MOOD='"+p+"'");
         ...
         rss=smt2.executeQuery("SELECT * FROM SONG WHERE S_ID='"+s_idf+"'");
                   while(rss.next())
                   { 
                       double mfcc1=rss.getDouble("MFCC");
                       double result1=0.0;
                       double result=0.0;
                       String s_ids=rss.getString("S_ID");
                       result=Math.pow((mfcc1-mfcc),2.0);
